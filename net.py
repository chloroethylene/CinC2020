...
def Netbeta(inputs, num_classes=9, include_top=True):    
    ...
    x = LeakyReLU(alpha=0.3)(x)
    x = Conv1D(12, 24, strides = 3, padding='same')(x)  
    x = LeakyReLU(alpha=0.3)(x)
    x = Dropout(0.2)(x)    
    ...
    x = Conv1D(12, 3, padding='same')(x)
    x = LeakyReLU(alpha=0.3)(x)
    x = Conv1D(12, 48, strides = 3, padding='same')(x)  
    x = LeakyReLU(alpha=0.3)(x)
    x = Dropout(0.2)(x)
...
def multi_input_model(num_classes=9, input_length=15360, nChannels=1):
    ...
    for i in range(12):
        ...
        locals()['x'+str(i+1)] = Netbeta(inputs=locals()['input'+str(i+1)], num_classes=num_classes, include_top=False)
        if i==0:
            x = locals()['x1']
        else:
